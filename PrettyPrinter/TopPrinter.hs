...
ppMemInit :: MemInit -> String
ppMemInit MemInitDef = "1, 1, 0, \"\", 0, 0"
ppMemInit (MemInitFunc _ (Build_VerilogMem ascii name offset size)) = "1, 0, " ++ if ascii then "1, " else "0, \"" ++ name ++ "\", " ++ show offset ++ ", " ++ show size
...
ppTop mod@((Build_ModDecl regs mems regUs memUs sends recvs, tmps), _) =
  ...
  ++ concatMap (\(i, (s, ((n, k), p))) -> ppIndent 1 ++ "verilog_mem#(" ++ show n ++ ", " ++ show (size k) ++ ", " ++ show p ++ "0, 0, 0, \"\", 0, 0);\n") (tag memUs)
  ++ ppIndent 1 ++ "design d(\n"
  ...
  ++ concatMap (\(i, (s, ((n, k), p))) -> ppIndent 2 ++ ".decl_" ++ ppMem "URq" (s, i) ++ "(" ++ ppMem "URq" (s, i) ++ "),\n") (tag memUs)
  ++ concatMap (\(i, (s, ((n, k), p))) -> ppIndent 2 ++ ".decl_" ++ ppMem "URqEn" (s, i) ++ "(" ++ ppMem "URqEn" (s, i) ++ "),\n") (tag memUs)
  ++ concatMap (\(i, (s, ((n, k), p))) -> ppIndent 2 ++ ".decl_" ++ ppMem "UWrIdx" (s, i) ++ "(" ++ ppMem "UWrIdx" (s, i) ++ "),\n") (tag memUs)
  ++ concatMap (\(i, (s, ((n, k), p))) -> ppIndent 2 ++ ".decl_" ++ ppMem "UWrVal" (s, i) ++ "(" ++ ppMem "UWrVal" (s, i) ++ "),\n") (tag memUs)
  ++ concatMap (\(i, (s, ((n, k), p))) -> ppIndent 2 ++ ".decl_" ++ ppMem "UWrEn" (s, i) ++ "(" ++ ppMem "UWrEn" (s, i) ++ "),\n") (tag memUs)
...
ppTb mod@((Build_ModDecl regs mems regUs memUs sends recvs, tmps), _) =
  ...
  ++ concatMap (\(i, (s, ((n, k), p))) -> ppIndent 2 ++ ppMem "URp" (s, i) ++ " = " ++ ppRandom (size k) ++ ";\n") (tag memUs)
