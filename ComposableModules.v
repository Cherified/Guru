...
Definition MethInclusion {t1 t2: Tree Elem} {K_in K_out: Kind}
  ...
  forall (v_in : type K_in),
    ActionInclusion (M1 v_in) (M2 v_in) rel.

Fixpoint Rel (mt : ModuleType)
  ...
  | MkMod => fun m1 m2 =>
      ModInclusion m1 m2 (fun s1 s2 => rel_acc s1.(Fst) s2.(Fst) /\ rel_curr s1.(Snd).(Fst) s2.(Snd).(Fst))
  | MkMeth K_in K_out => fun m1 m2 =>
      MethInclusion m1 m2 rel_curr
  | BindMod B => fun F1 F2 =>
      forall (t_new1 t_new2 : Tree Elem)
             (m1_in : Mod t_new1)
             (m2_in : Mod t_new2)
             (rel_new : TreeState ElemState t_new1 -> TreeState ElemState t_new2 -> Prop),
        ModInclusion m1_in m2_in rel_new ->
        Rel B (Node "" [t_acc1; t_curr1]) (Node "" [t_acc2; t_curr2])
  ...
  | BindMeth K_in K_out B => fun F1 F2 =>
      forall (M1 : type K_in -> Action type t_curr1 K_out) (M2 : type K_in -> Action type t_curr2 K_out),
        MethInclusion M1 M2 rel_curr ->
        Rel B t_acc1 t_acc2 rel_acc t_curr1 t_curr2 rel_curr (F1 M1) (F2 M2)
  end m1 m2.
